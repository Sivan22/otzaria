# thid script creates markdown files from sefaria json files

# sefaria_export_json_files_path = 
...
def to_gematria(i)->str:
    ...
    j = i/1000
    if j<0:
        s = int_to_gematria(j, gershayim=False)+ ' אלפים '
        ...
        if depth == 0:
            ...
            for i, item in enumerate(text, start=1):
                if item != [] and item != [[]]:
                    ...
                    if depth>1:                        
                        globals()['output'].append(f"{codes[level]} {section_names[-depth]} {letter}\n")
...
def process_node(node, text,level=0):
    ...
    if 'nodes' in node:  # Process nested nodes
        ...
        globals()['output'].append(f"{codes[level]} {node_title}\n")
...
def process_complex_book(text_file_name, schema_file_name, output_file_name):
    ...
    # add book title
    globals()['output'].append(f'# {index["schema"]["heTitle"]}\n')
    # add authors name
    if 'authors' in index:
