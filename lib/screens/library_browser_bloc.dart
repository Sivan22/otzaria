...
class LibraryBrowserBloc extends StatefulWidget {
  const LibraryBrowserBloc({Key? key}) : super(key: key);

  @override
  State<LibraryBrowserBloc> createState() => _LibraryBrowserBlocState();
}

class _LibraryBrowserBlocState extends State<LibraryBrowserBloc>
      ...
      if (entry is ExternalBook) {
        ...
        items.add(
          BookGridItem(
            ...
            onBookClickCallback: () {
              if (entry is PdfBook) {
                context.read<TabsBloc>().add(AddTab(PdfBookTab(entry, 1)));
              } else if (entry is TextBook) {
                context.read<TabsBloc>().add(AddTab(
                    TextBookTab(book: entry, index: 0, openLeftPane: true)));
              }
            },
          ),
        );
            ...
            BookGridItem(
                ...
                onBookClickCallback: () {
                  if (book is PdfBook) {
                    ...
                    context.read<TabsBloc>().add(AddTab(
                        TextBookTab(book: book, index: 0, openLeftPane: true)));
                  }
                }),
          );
        }
